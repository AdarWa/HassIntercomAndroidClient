...
class HomeAssistantClient(
  ...
  private suspend fun registerClient() {
    connection?.send(
        mapOf("type" to "register", "role" to "home_assistant", "client_id" to clientId))
  ...
  private suspend fun handleResponse(msg: JSONObject) {
    val commandId = msg.optString("command_id")
    ...
    if (!status.equals("ok", ignoreCase = true)) {
      val explicitError = msg.optString("payload").takeIf { it.isNotBlank() }
      val message = explicitError ?: status.ifBlank { "Server reported an error" }
      throw IOException("Command ${pendingCmd.command} failed: $message")
    }

    when (pendingCmd.command) {
      "start_audio" -> startStreamFromResponse(payload)
      "stop_audio" -> payload["stream_id"]?.let { terminateStream(it.toString()) }
    }
  }

  private fun startStreamFromResponse(payload: Map<String, Any>) {
  ...
  private suspend fun handleError(msg: JSONObject): Nothing {
    Log.e("HAClient", msg.toString())
    val commandId = msg.optString("command_id")
    val pendingCmd = if (commandId.isNotBlank()) pending.remove(commandId) else null
    val details = msg.optJSONObject("details")?.toMap() ?: emptyMap()
    val reason = msg.optString("reason") ?: "Unknown Reason"
    val streamId = details["stream_id"] as? String
    streamId?.let { terminateStream(it) }

    val messageParts = mutableListOf<String>()
    msg.optString("payload").takeIf { it.isNotBlank() }?.let { messageParts.add(it) }
    msg.optString("error").takeIf { it.isNotBlank() }?.let { messageParts.add(it) }
    msg.optString("reason").takeIf { it.isNotBlank() }?.let { messageParts.add(it) }
    msg.optString("status").takeIf { it.isNotBlank() }?.let { messageParts.add("status $it") }

    if (messageParts.isEmpty()) {
      messageParts.add("Server reported an error")
    }

    val detailText = if (details.isNotEmpty()) " Details: $details" else ""
    val prefix = pendingCmd?.let { "Command ${it.command} failed. " } ?: ""
    throw IOException(prefix + messageParts.joinToString(". ") + detailText)
  }
