...
class MainActivity : ComponentActivity() {
    ...
    fun IntercomUI() {
        ...
        Column(
            ...
            OutlinedTextField(value = clientId, onValueChange = { clientId = it }, label = { Text("Client ID") }, singleLine = true)

            Text("Audio Format", style = MaterialTheme.typography.titleMedium)
            OutlinedTextField(value = sampleRate, onValueChange = { sampleRate = it.filter { c -> c.isDigit() } }, label = { Text("Sample Rate") }, singleLine = true)
            OutlinedTextField(value = channels, onValueChange = { channels = it.filter { c -> c.isDigit() } }, label = { Text("Channels") }, singleLine = true)
            OutlinedTextField(value = frameMs, onValueChange = { frameMs = it.filter { c -> c.isDigit() } }, label = { Text("Frame Size (ms)") }, singleLine = true)
